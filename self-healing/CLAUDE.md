...
## Prostředí
- **Dokploy** spravuje 7 webových apek (lokwave platforma): appName vzory `lokwave-vet-*`, `app-*` (salon/bistro/fit/auto/dental/lokwave). Deploy přes REST API: base `http://localhost:3000/api`, header `x-api-key` z `/srv/homelab/secrets/dokploy-api-token.txt`. Klíčové appId: vet=nFheBME6Eq1lXoi6zF_Kb, salon=GZiwn5juiAs0xASc-Jjmi, bistro=Nyu0LW2QR4stN2-muVYCA, fit=jDYgVQoiWLxuG1rRH49j0, auto=LXKaQIyUM8zC2Qo7Dg-o8, lokwave=hQ8Gt3fTusi56xHj9DZAs, dental=qaVK0Be6sjWLLCFnu6iKU. Redeploy: `POST application.deploy {"applicationId":"..."}`. Stav: `GET application.one?applicationId=...` → applicationStatus.
