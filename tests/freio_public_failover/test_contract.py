...
class FreioPublicFailoverContractTest(unittest.TestCase):
    ...
    def test_shell_and_systemd_contracts_are_fail_closed(self) -> None:
        ...
        self.assertIn("x-freio-edge-fallback: static-v1", script.lower())
        self.assertIn("x-freio-edge-fallback: health-v1", script.lower())
        self.assertIn("/__freio-edge-health", script)
        self.assertIn("EDGE_ENABLED_MARKER", script)
        self.assertIn("edge-fallback", script)
        self.assertIn("previous_route", script)
        self.assertIn('persist_route "$incident_state"', script)
        self.assertIn('incident_state=edge-fallback', script)
