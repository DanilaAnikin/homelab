...
class PostizBackupContractTests(unittest.TestCase):
    ...
    def test_capture_has_real_writer_fence_and_crash_recovery(self) -> None:
        ...
        self.assertIn("write-compose-no-deps-model", self.capture)
        self.assertIn("config --hash postiz", self.capture)
        self.assertIn("--resolved-compose-hashes", self.capture)
        self.assertIn("--postiz-no-deps-compose-json", self.capture)
        self.assertIn("--postiz-no-deps-hash", self.capture)
        self.assertIn("docker image inspect", self.capture)
        self.assertIn("--image-inspect-json", self.capture)
        self.assertIn("docker network inspect", self.capture)
        self.assertIn("--network-inspect-json", self.capture)
        self.assertIn("--expected-image", self.capture)
        self.assertEqual(self.capture.count("--expected-image"), 4)
        self.assertIn("--runtime-state \"$suffix\"", self.capture)
